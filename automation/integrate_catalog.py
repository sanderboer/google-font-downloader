...
def update_fontdownloader_cli(cli_path: Path, catalog: Dict, max_families: Optional[int] = None, backup: bool = True):
        ...
        if hasattr(cli_module, '_get_google_fonts_api_data'):
            try:
                ...
                if 'items' in fallback_data and len(fallback_data['items']) > 0:
                    print(f"✅ Integration verified: {len(fallback_data['items'])} families loaded")
                    return True
...
def main():
    parser = argparse.ArgumentParser(description='Integrate catalog into fontdownloader')
    parser.add_argument('catalog', help='Path to generated catalog JSON')
    parser.add_argument('--cli-path', default='../fontdownloader/cli.py',
                      help='Path to fontdownloader CLI file')
    parser.add_argument('--max-families', type=int,
                      help='Limit number of families to include')
    parser.add_argument('--no-backup', action='store_true',
                      help='Skip creating backup of original CLI file')
    parser.add_argument('--verify', action='store_true',
                      help='Verify integration after update')
    ...
    print(f"🔧 Integrating catalog into fontdownloader")
