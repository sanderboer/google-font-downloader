...
class GoogleFontsCatalogBuilder:
    ...
    def get_font_directories(self, license_type: str) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"Found {len(directories)} {license_type} font families")
                return directories
            else:
                self.logger.error(f"GitHub API error for {license_type}: {response.status_code}")
                return []
    ...
    def parse_metadata_pb(self, license_type: str, font_slug: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if category_match:
                    metadata['category'] = category_match.group(1).lower().replace('_', '-')
    ...
    def _guess_category(self, font_slug: str) -> str:
        ...
        serif_indicators = ['serif', 'times', 'georgia', 'crimson', 'merriweather', 'playfair', 'lora']
        mono_indicators = ['mono', 'code', 'inconsolata', 'courier', 'source code', 'jetbrains']
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                font_files = [
                    ...
                    if item['type'] == 'file' and item['name'].endswith(('.ttf', '.otf', '.ttc'))
                ]
                return font_files
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200 and '@font-face' in response.text:
                    ...
                    if variants:
                        self.logger.debug(f"Found {len(variants)} variants for {family_name}")
                        return variants
                ...
                self.logger.debug(f"CSS2 attempt {attempt + 1} failed for {family_name}: {e}")
    ...
    def build_font_entry(self, license_type: str, font_slug: str) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.info(f"✅ {metadata['name']} - {len(variants)} variants, {len(files)} files")
            return entry
    ...
    def build_complete_catalog(self, max_fonts_per_license: Optional[int] = None) -> Dict:
    ...
    def _print_stats(self, catalog: Dict):
        ...
        self.logger.info(f"   Success rate: {self.stats['successful_families']}/{self.stats['total_families']} ({100*self.stats['successful_families']/max(self.stats['total_families'],1):.1f}%)")
        self.logger.info(f"   Generation time: {catalog['meta']['generation_time_seconds']}s")
        self.logger.info(f"   API calls: GitHub={self.stats['api_calls']['github']}, CSS2={self.stats['api_calls']['css2']}, Metadata={self.stats['api_calls']['metadata']}")


def main():
    parser = argparse.ArgumentParser(description='Build complete Google Fonts catalog')
    parser.add_argument('--output', '-o', default='google_fonts_catalog.json',
                      help='Output file path (default: google_fonts_catalog.json)')
    parser.add_argument('--max-fonts', type=int,
